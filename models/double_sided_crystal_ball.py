...
class DoubleSidedCrystalBall_EGM(torch.distributions.Distribution):
    """
    Double Crystal Ball distribution.
    ...
    Args:
        mu (torch.Tensor): Mean of the distribution.
        sigma (torch.Tensor): Standard deviation of the distribution.
        alpha_l and alpha_r > 0, 
        eta_l and eta_r > 1
        sigma > 0
        
        EGM-22-001

        The pdf is normalized in [-inf,inf].

    """

    def __init__(self, mu, sigma, alphaL, alphaR, etaL, etaR, xmin=-10, xmax=10):
        super().__init__()
        self.mu = torch.as_tensor(mu)
        self.sigma = torch.as_tensor(sigma)
        self.alphaL = torch.as_tensor(alphaL)
        self.alphaR = torch.as_tensor(alphaR)
        self.etaL = torch.as_tensor(etaL)
        self.etaR = torch.as_tensor(etaR)
        self.xmax = torch.as_tensor(xmax)
        self.xmin = torch.as_tensor(xmin)
    ...
    def _calculate_normalization_constant(self):
        """Calculates the normalization constant for one side of the Crystal Ball."""
        PI = torch.ones_like(self.etaL)*torch.pi
        A = (self.etaL/(self.alphaL*(self.etaL-1)))*torch.exp(-self.alphaL**2/2)+\
                    (torch.sqrt(PI)/2) * (torch.erf(self.alphaL/(2**0.5)) + torch.erf(self.alphaR/(2**0.5))) +\
                    (self.etaR/(self.alphaR*(self.etaR-1)))*torch.exp(-self.alphaR**2/2)
        return  A*(self.sigma)
    ...
    def log_prob(self, x):
        ...
        if x.ndim == 0:
            x = torch.tensor([x])
        if x.ndim < 2:
            z = (x.unsqueeze(-1) - self.mu) / self.sigma
        else:
            z = (x - self.mu) / self.sigma
        
        # Use vectorized operations for efficiency
        left_mask = z < -self.alphaL
        right_mask = z > self.alphaR
        gaus_mask = ~(left_mask | right_mask)  # Not left or right tail

        log_prob = torch.full_like(z, -torch.inf)
        if self.alphaL.ndim == 0:
            alphaL_e = self.alphaL.expand(z.shape[0],1)
            alphaR_e = self.alphaR.expand(z.shape[0],1)
            etaL_e = self.etaL.expand(z.shape[0],1)
            etaR_e = self.etaR.expand(z.shape[0],1)
            n_e = self.log_normalization.expand(z.shape[0],1)
        ...
        log_prob[left_mask] =  n_e[left_mask] - (alphaL_e[left_mask]**2/2) + \
                            -etaL_e[left_mask]*torch.log( (alphaL_e[left_mask]/etaL_e[left_mask])*((etaL_e[left_mask]/alphaL_e[left_mask])-alphaL_e[left_mask] - z[left_mask]))
        
        log_prob[right_mask] = n_e[right_mask] - (alphaR_e[right_mask]**2/2) + \
                            -etaR_e[right_mask]*torch.log( (alphaR_e[right_mask]/etaR_e[right_mask])*((etaR_e[right_mask]/alphaR_e[right_mask])-alphaR_e[right_mask] + z[right_mask]))
        
        log_prob[gaus_mask] =  n_e[gaus_mask] - (z[gaus_mask]**2 / 2)
