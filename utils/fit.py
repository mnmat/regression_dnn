...
def get_mean_and_std(h):
    mean = np.average(h.axes[0].centers, weights=h.values())
    stdDev = np.sqrt(np.average((h.axes[0].centers-mean)**2, weights=h.values()))
    return mean, stdDev

def get_median_and_iqr(h):
    cum_counts = np.cumsum(h.values())
    total = cumsum[-1]

    median_idx = np.searchsorted(cum_counts, total/2)
    median = h.axes[0].centers[median_idx]

    # IQR (25% and 75%)
    q1_idx = np.searchsorted(cum_counts, total*0.25)
    q3_idx = np.searchsorted(cum_counts, total*0.75)
    iqr = h.axes[0].centers[q3_idx] - h.axes[0].centers[q1_idx]
    return median, iqr


def fit_gaussian(h):
    mean, stdDev = get_mean_and_std(h)
    params, cov = curve_fit(gaussian,h.axes[0].centers, h.values(),
        p0 = [np.max(h_mu),mean,stdDev],
